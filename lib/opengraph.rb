...
module OpenGraph
  ...
  def self.parse(html, strict = true)
    ...
    namespaces = doc.css('head').attribute('prefix').to_s.scan(/\s*(.+?): http[^#]+#/).flatten
    doc.css('meta').each do |m|
      if m.attribute('property') && m.attribute('property').to_s.match(/^((?:#{namespaces.join('|')}):.+)$/i)
        key = $1.gsub('-','_').gsub(/^og:/, '') # strip og: from og namespace properties
        page[key] = m.attribute('content').to_s
      end
    end
